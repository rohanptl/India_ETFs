...
DEFAULT_BENCHMARK_HURDLE = "nifty"
...
def select_candidates(
    ...
    for idx, row in df.iterrows():
        ...
        if ok and pd.notna(row["ret_6m"]):
            ok = float(row["ret_6m"]) > hurdle
    ...
    if prev_holdings:
        ...
        keep_threshold = max(
            top_k,
            int(hold_band) if hold_band is not None else top_k + DEFAULT_HOLD_BAND_OFFSET,
        )
