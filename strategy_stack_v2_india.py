...
DEFAULT_MAX_WAIT_PULLBACK = 3
DEFAULT_TRANSACTION_COST_BPS = 0
COMPARISON_BENCHMARK_TICKERS = {"NIFTYBEES.NS", "SENSEXBEES.BO"}
...
def parse_universe_file(path: str) -> List[str]:
    ...
    with open(path, "r", encoding="utf-8", errors="ignore") as f:
        for raw in f:
            ...
            if not line:
                continue
            if re.fullmatch(r"[A-Z0-9]{1,20}(?:[.-][A-Z0-9]{1,10})?", line):
                if line not in seen:
                    seen.add(line)
                    tickers.append(line)
    if not tickers:
...
def download_ohlcv_history(
    ...
    print_progress(f"Downloading {len(missing)} missing tickers  |  {start or 'max'} → {end or 'latest'}")
...
def build_snapshot_metrics(
    ...
    numeric_cols = [
        "last_close", "sma50", "sma200", "ema10", "macd",
        "macd_signal", "macd_hist", "weekly_macd_hist", "atr15",
        "realized_vol20", "ret_6m",
    ]
...
def _add_eligibility(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["eligible"] = df["cash_like"] | (non_cash & df["above_sma200"].astype(bool))
    return df
...
def _add_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if eligible_mask.any() and df.loc[eligible_mask, "ret_6m"].notna().any():
        ...
        score[idx] += ((df.loc[eligible_mask, "ret_6m"] - mn) / (mx - mn + 1e-12)).to_numpy() * 5.0
    ...
    if eligible_mask.any() and df.loc[eligible_mask, "realized_vol20"].notna().any():
        ...
        idx = df.loc[eligible_mask].index
        score[idx] += (1.0 - (df.loc[eligible_mask, "realized_vol20"] - mn) / (mx - mn + 1e-12)).to_numpy() * 1.0
...
def apply_entry_labels_and_allocate(
    ...
    if not wait_pb.empty:
        wait_pb = wait_pb.sort_values(["model_target_weight", "raw_score", "ticker"], ascending=[False, False, True])
...
def run_schedule_backtest(
    ...
    print_progress(f"{schedule_name}: {len(rebalance_dates)} rebalances, {len(entry_check_dates)} weekly entry checks")
    ...
    for i, dt in enumerate(px.index):
        ...
        if need_rebalance or need_entry_check:
            ...
            turnover_rows.append({"date": dt, "turnover": turnover, "event": "rebalance" if need_rebalance else "entry_check"})
            ...
            entry_lbl_hist.append({
                ...
                "cash_weight": float(alloc.loc[alloc["ticker"] == cash_ticker, "target_weight"].sum()),
            })
    ...
    return (
        pd.Series(equity_vals, index=px.index),
...
def allocation_mode(
    ...
    actions_view = actions_view[~((actions_view["current_alloc_pct"].abs() < 1e-9) & (actions_view["target_alloc_pct"].abs() < 1e-9))]
...
def backtest_mode(
    ...
    print_progress(
        f"Universe: {len(universe)} ETFs from file  |  Backtest: {start} → {end}  |  "
        f"Download from: {buffered_start}"
    )
    ...
    for label, code in schedules.items():
        result = run_schedule_backtest(
            universe, ohlcv, close_px, feature_store, code, cash_ticker, top_k,
            start, end, hold_band, max_alloc, transaction_cost_bps,
            allocation_mode, execution_mode, max_wait_pullback,
        )
        if result[0] is None:
        ...
        stats.update({
            ...
            "Avg Cash Weight": float(e_df["cash_weight"].mean()) if not e_df.empty else 0.0,
            "Schedule": label,
        })
    ...
    summary = pd.DataFrame(summary_rows)[[
        ...
        "Avg Turnover/Trade Date", "Num Trade Dates", "Avg BUY NOW Count", "Avg WAIT Count",
        "Avg DO NOT BUY", "Avg Cash Weight",
    ...
    print("=" * 126)
    print("BACKTEST SUMMARY: DRIFT-AWARE MOC | WEEKLY vs MONTHLY vs QUARTERLY vs BENCHMARKS (NIFTYBEES.NS, SENSEXBEES.BO)")
    print("=" * 126)

    disp = summary.copy()
    for c in ["Total Return", "CAGR", "Annual Vol", "Max Drawdown", "Avg Turnover/Trade Date", "Avg Cash Weight"]:
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Indian ETF Strategy Stack — allocation & backtest tool")
    ...
    p.add_argument(
        ...
        help="Maximum allocation per non-cash ETF as a 0-1 fraction.",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help="overlay = obey BUY NOW / WAIT FOR PULLBACK labels; pure_topk = use full model weights directly.",
    )
    p.add_argument(
        "--max-wait-pullback",
        type=int,
        default=DEFAULT_MAX_WAIT_PULLBACK,
